...
**数据来源**：
- 存储位置：SQLite数据库（`data.db`）
- 数据表：`articles` 表，category字段为 `programming`
- 数据格式：每个文章包含标题、URL、摘要、来源、发布时间等
...
**数据来源**：
- 存储位置：SQLite数据库（`data.db`）
- 数据表：`articles` 表，category字段为 `ai_news`
- 数据格式：与编程资讯相同
...
2. 系统处理：
   ...
   - 保存到候选池（`data/articles/ai_candidates.json`，JSON文件仅用于候选数据）
   - 等待管理员审核
3. 用户提示：
   - 显示"提交成功"消息
...
**数据来源**：
   - 存储位置：SQLite数据库（`data.db`）
   - 数据表：`tools` 表，is_featured字段为 `True`
   - 数据格式：每个工具包含ID、名称、URL、描述、分类、标签、图标、访问量等
...
**数据来源**：
   - 存储位置：SQLite数据库（`data.db`）
   - 数据表：`tools` 表，按category字段分类
   - 每个分类在数据库中通过category字段区分
...
2. 系统处理：
   - 检查URL是否重复（候选池和正式工具池）
   - 保存到工具候选池（`data/tools/tool_candidates.json`，JSON文件仅用于候选数据）
   - 等待管理员审核
3. 用户提示：
   - 显示"提交成功"消息
   - 说明工具将人工审核
...
### 后端技术栈
- **框架**：FastAPI
- **数据存储**：SQLite数据库（`data.db`），候选数据使用JSON文件（`data/` 目录）
